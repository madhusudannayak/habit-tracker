package com.example.habittracker.ui.fragment.updatehabit
...
class UpdateHabitItem : Fragment(R.layout.fragment_update_habit_item),
    ...
    private fun updateHabitToDB() {
        title = et_habitDescription_update.text.toString()
        description = et_habitDescription_update.text.toString()
        timeStamp = "$cleanDate $cleanTime"

        if (!(title.isEmpty() || description.isEmpty() || timeStamp.isEmpty() || drawableSelected == 0)) {
            val habit =
                Habits(args.selectedHabit.id, title, description, timeStamp, drawableSelected)
            habitViewModel.updateHabit(habit)
            Toast.makeText(context, "Habit updated successfully", Toast.LENGTH_SHORT).show()
            findNavController().navigate(R.id.action_updateHabitItem_to_habitListFragment)
        } else {
    ...
    private fun deleteHabit(habit: Habits) {
